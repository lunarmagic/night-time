...
namespace night
{
	...
	u8 Camera::should_cull_plane(vec3 const& point_on_plane, vec3 const& plane_normal) const
	{
		if (type == ECameraType::Perspective)
		{
			...
			real d = math::dot(plane_normal, ab);
			return (d > 0);
		}
		else if (type == ECameraType::Orthographic)
		{
			return (math::dot(plane_normal, direction()) > 0);
		}
	...
	{
		return math::normalize(math::cross(p2 - p1, p3 - p2));
	}
	...
	Camera Camera::operator*(CameraTransform const& x) const
	{
		...
		auto rotation = math::rotate_about_vector(NIGHT_CAMERA_DEFAULT_DIRECTION, rhs_direction);
